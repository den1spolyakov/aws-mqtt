...
exports.createCanonicalRequest = function (method, pathname, query, headers, payload, doubleEscape) {
  return [method.toUpperCase(), createCanonicalURI(doubleEscape ? pathname.split(/\//g).map(function (v) {
    return encodeURIComponent(v);
  }).join("/") : pathname), exports.createCanonicalQueryString(query), exports.createCanonicalHeaders(headers), exports.createSignedHeaders(headers), createCanonicalPayload(payload)].join("\n");
}; /* eslint-env node */
...
function createCanonicalURI(uri) {
  var url = _pathBrowserify2.default.resolve(uri);
...
exports.createCanonicalQueryString = function (params) {
  ...
  if (typeof params == "string") {
    params = _querystringEs2.default.parse(params);
  }
...
exports.createCanonicalHeaders = function (headers) {
  return Object.keys(headers).sort().map(function (name) {
    ...
    return name.toLowerCase().trim() + ":" + values.map(function (v) {
...
exports.createAuthorizationHeader = function (key, scope, signedHeaders, signature) {
...
exports.createPresignedURL = function (method, host, path, service, payload, options) {
  ...
  var query = options.query ? _querystringEs2.default.parse(options.query) : {};
  ...
  var canonicalRequest = exports.createCanonicalRequest(method, path, query, options.headers, payload, options.doubleEscape);
  ...
  return options.protocol + "://" + host + path + "?" + _querystringEs2.default.stringify(query);
};
...
function hmac(key, string, encoding) {
  return _awsSdkReactNative2.default.util.crypto.hmac(key, string, encoding);
}

function hash(string, encoding) {
  return _awsSdkReactNative2.default.util.crypto.sha256(string, encoding);
}